...
#include "Models/rf-nodetype.h"
#include "Models/rf-children.h"
#include "Models/rf-pathtoroot.h"
#include "Models/rf-nodetype-children.h"
#include "Models/rf-nodetype-children-pathtoroot.h"
...
enum class Model {
  NONE=0,
  RF_RM,//
  RF_FULL_TOROOT_BITVECTOR,//
  RF_CHILDREN,//
  RF_RM_CHILDREN,//
  RF_RM_CHILDREN_FULL_TOROOT_BITVECTOR//
};

static cl::opt<Model> machineLearningModelOpt {"model",
  cl::desc{"Choose the machine learning model to use in the reduction process."},
  cl::values(
    clEnumValN(Model::NONE, "none",
      "No model."),
    clEnumValN(Model::RF_RM, "rf-type",
      "Random forest with type of query node as feature."),
    clEnumValN(Model::RF_FULL_TOROOT_BITVECTOR, "rf-pathtoroot",
      "Random forest with bitvector of node types on full path from the query node to root as feature."),
    clEnumValN(Model::RF_RM_CHILDREN_FULL_TOROOT_BITVECTOR, "rf-type-children-pathtoroot",
      "Random forest with type of query node, bitvector of query node's children types and bitvector of full path from query node to root as features."),
    clEnumValN(Model::RF_CHILDREN, "rf-children",
      "Random forest with types of children of query node as features."),
    clEnumValN(Model::RF_RM_CHILDREN, "rf-type-children",
      "Random forest with type of query node and type of its children as features.")
  ),
  cl::cat{reducerCategory},
  cl::init(Model::NONE)};

 static void
selectMLModel(std::string & model_name) {
  switch (machineLearningModelOpt) {
  case Model::RF_RM:
    model_name = "rf_rm";
    break;
  case Model::RF_FULL_TOROOT_BITVECTOR:
    model_name = "rf_full_toroot_bitvector";
    break;
  case Model::RF_RM_CHILDREN_FULL_TOROOT_BITVECTOR:
    model_name = "rf_rm_children_full_toroot_bitvector";
    break;
  case Model::RF_CHILDREN:
    model_name = "rf_children";
    break;
  case Model::RF_RM_CHILDREN:
    model_name = "rf_rm_children";
    break;
  case Model::NONE:
  default:
    break;
...
using N = testshrinker::TreeNode<const antlr4::tree::ParseTree*>*;

int
getNodeID(N node) {
  auto ruleIndex = getRuleIndex(node->getElement());
  if (ruleIndex.first) {
      auto r = ruleIndex.second;
      return r;
  }
  return -1;
}
...
std::vector<N>
getPathFromNodeToRoot(N node) {
  std::vector<N> node_to_root;
  N on_path = node;
  node_to_root.push_back(on_path);
  auto parent = on_path->getParent();
  while (parent) {
      on_path = parent;
      node_to_root.push_back(on_path);
      parent = on_path->getParent();
  }
  return node_to_root;
}
...
struct rm_feat {
  std::vector<float>
  extractFeatures(N node, testshrinker::Tree<const antlr4::tree::ParseTree*>& tree) {
    std::vector<float> features;
    auto rm_type_id = getNodeID(node);
    assert(rm_type_id != -1);
    features.push_back(rm_type_id);
    return features;
  }

};

struct children_feat {
  std::vector<float>
  extractFeatures(N node, testshrinker::Tree<const antlr4::tree::ParseTree*>& tree) {
    // 203 internal type rules + terminal
    std::vector<float> features(204, 0);
    auto children = node->getChildren();
    auto null = std::remove(children.begin(), children.end(), nullptr);
    children.erase(null, children.end());
    for(auto& child: children) {
      auto child_type_id = getNodeID(child);
      // It's a terminal
      if(child_type_id == -1) {
        features[203] = 1;
      }
      else {
        features[child_type_id] = 1;
      }
    }
    return features;
  }
...
struct full_toroot_bitvector_feat {
  std::vector<float>
  extractFeatures(N node, testshrinker::Tree<const antlr4::tree::ParseTree*>& tree) {
    // cpnf grammar has 203 rule types.
    std::vector<float> features(203, 0);
    auto node_to_root = getPathFromNodeToRoot(node);
    // keeps ids of nodes on path from node to root
    std::vector<int> nodeIds;
    for(auto& nd: node_to_root) {
      auto id = getNodeID(nd);
      assert(id != -1);
      nodeIds.push_back(id);
    }
    // root id (pnf_start) is 173
    assert(nodeIds[nodeIds.size()-1] == 173);
    for(auto & id : nodeIds) {
      features[id] = 1;
    }
    return features;
...
struct rm_children_feat {
  std::vector<float>
  extractFeatures(N node, testshrinker::Tree<const antlr4::tree::ParseTree*>& tree) {
    // features related to type id of rm node
    rm_feat rm;
    auto rm_features = rm.extractFeatures(node, tree);
    // features related to rm children type ids
    children_feat ch;
    auto ch_features = ch.extractFeatures(node, tree);
    rm_features.insert(rm_features.end(), ch_features.begin(), ch_features.end());
    return rm_features;
  }

};

struct rm_full_toroot_bitvector_feat {
  std::vector<float>
  extractFeatures(N node, testshrinker::Tree<const antlr4::tree::ParseTree*>& tree) {
    full_toroot_bitvector_feat f;
    auto features = f.extractFeatures(node, tree);
    // features related to type id of rm node
    rm_feat rm;
    auto rm_features = rm.extractFeatures(node, tree);
    features.insert(features.end(), rm_features.begin(), rm_features.end());
    return features;
  }

};

struct rm_children_full_toroot_bitvector_feat {
  std::vector<float>
  extractFeatures(N node, testshrinker::Tree<const antlr4::tree::ParseTree*>& tree) {
    rm_full_toroot_bitvector_feat f;
    auto features = f.extractFeatures(node, tree);
    // features related to type id of rm node
    rm_feat rm;
    auto rm_features = rm.extractFeatures(node, tree);
    ...
    auto ch_features = ch.extractFeatures(node, tree);
    rm_features.insert(rm_features.end(), ch_features.begin(), ch_features.end());
    rm_features.insert(rm_features.end(), features.begin(), features.end());
    return rm_features;
  }
...
struct PriorityQueueMLReducerImpl {
  AntlrGrammarInfo const& grammar;
  antlr4::Parser& parser;
  PropertyChecker& checker;
  TimeoutGuard timer;
  ...
  PriorityQueueMLReducerImpl(AntlrGrammarInfo const& grammar,
                           antlr4::Parser& parser,
                           PropertyChecker& checker,
                           T & model,
                           U & testData)
  ...
  operator()(testshrinker::Tree<E>& tree, const size_t targetLevel = -1) {
    ...
    Weight root_weight {0,0,0};
    worklist.push({tree.root, root_weight});
    auto processor = std::make_unique<NodePolicy<E>>(tree, checker);
    while (!worklist.empty()) {
      if (timer.hasTimedOut()) {
        llvm::errs() << "TIMED OUT!\n";
        break;
      }
      processor->processWork(worklist, model, testData, tree);
    }
...
private:
  ...
  public:
    ...
    void
    processWork(Worklist<E>& worklist, T & model, U & testData, testshrinker::Tree<E>& tree) {
      processWorkImpl(worklist, model, testData, tree);
    }
  ...
  private:
    ...
    virtual void processWorkImpl(Worklist<E>& worklist, T & model, U & testData, testshrinker::Tree<E>& tree) = 0;
    virtual Weight makeWeight(Node<E>* node) = 0;
  };
  ...
  private:
    void
    processWorkImpl(Worklist<E>& worklist, T & model, U & testData, testshrinker::Tree<E>& tree) override {
      auto work = worklist.top();
      worklist.pop();
      if (work.node->isNullable()) {
        bool EXECUTE = true;
        auto features = testData.extractFeatures(work.node, tree);
        if(features.size() == 1 && features[0] == -1) {
          // new path
        }
        else {
          float features_array[features.size()];
          std::copy(features.begin(), features.end(), features_array);
          auto predicted_val = model.predict(features_array);
          // There are two prediction classes: 0 that is unsuccessful oracl run and 1 that is successful.
          EXECUTE = (predicted_val == 1);
        }
        if(EXECUTE && this->removalSucceeds(work.node)) {
          this->remove(work.node);
        }
        else {
          this->addWork(worklist, work.node->getChildren());
...
void
AntlrPriorityQueueMLReducer::reduce(ShrinkerTree& tree,
                                  antlr4::Parser& parser,
                                  AntlrGrammarInfo const& grammar) {
  auto checker  = PropertyChecker{listener, propertyChecker, grammar, tree};
  std::string model_name = "";
  selectMLModel(model_name);
   // No model so it is Pardis reduction
    if(model_name == "") {
      using Reducer = PriorityQueueReducerImpl;
      auto reducer  = Reducer{grammar, parser, checker};
      runToFixedPoint(listener, reducer, tree);
    ...
    else if(model_name == "rf_rm") {
      rm_feat testData;
      rf_rm_gcctorture model;
      using Reducer = PriorityQueueMLReducerImpl<rf_rm_gcctorture, rm_feat>;
      auto reducer  = Reducer{grammar, parser, checker, model, testData};
      reducer(tree);
      auto tokens = getTokens(tree);
      grammar.print(out, tokens);
      out << "\n";
    ...
    // feature is a vector of size 203 (number of rule types in grammar cpnf). element of vector is 1 if a rule type
     // is present on the path from rm to root. 0 otherwise.
      
    else if(model_name == "rf_full_toroot_bitvector") {
      full_toroot_bitvector_feat testData;
      rf_full_toroot_bitvector_gcctorture model;
      using Reducer = PriorityQueueMLReducerImpl<rf_full_toroot_bitvector_gcctorture, full_toroot_bitvector_feat>;
      auto reducer  = Reducer{grammar, parser, checker, model, testData};
      reducer(tree);
      auto tokens = getTokens(tree);
      grammar.print(out, tokens);
    ...
    else if(model_name == "rf_rm_children_full_toroot_bitvector") {
      rm_children_full_toroot_bitvector_feat testData;
      rf_rm_children_full_toroot_bitvector_gcctorture model;
      using Reducer = PriorityQueueMLReducerImpl<rf_rm_children_full_toroot_bitvector_gcctorture, rm_children_full_toroot_bitvector_feat>;
      auto reducer  = Reducer{grammar, parser, checker, model, testData};
      reducer(tree);
      auto tokens = getTokens(tree);
      grammar.print(out, tokens);
     ...
     else if(model_name == "rf_children") {
      children_feat testData;
      rf_children_gcctorture model;
      using Reducer = PriorityQueueMLReducerImpl<rf_children_gcctorture, children_feat>;
      auto reducer  = Reducer{grammar, parser, checker, model, testData};
      reducer(tree);
      auto tokens = getTokens(tree);
      grammar.print(out, tokens);
     ...
     else if(model_name == "rf_rm_children") {
      rm_children_feat testData;
      rf_rm_children_gcctorture model;
      using Reducer = PriorityQueueMLReducerImpl<rf_rm_children_gcctorture, rm_children_feat>;
      auto reducer  = Reducer{grammar, parser, checker, model, testData};
      reducer(tree);
      auto tokens = getTokens(tree);
      grammar.print(out, tokens);
...
void
AntlrPriorityQueueMLFixpointReducer::reduce(ShrinkerTree& tree,
                                          antlr4::Parser& parser,
                                          AntlrGrammarInfo const& grammar) {
